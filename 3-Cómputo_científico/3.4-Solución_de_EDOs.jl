...
# Tu código va aquí :)
function tamañoDePaso(t0, tf, N) #dar de alta la funcion
if isinteger(N) #si n es entero
	p=(tf-t0)/N #donde p=h y se aplica la formula anterior 
	return p #devuelve p
else #si no es entero n
	println("El número de pasos no es un valor entero")
end
end	

# ╔═╡ a2018fc3-4880-41f8-8217-eff54b939ec0
begin
	t0=0   #asiganamos el valor del tiempo inicial
	tf=40   #asiganamos el valor del tiempo final
	N=100 #num pasos
		if N>0 #si n>0
	h=tamañoDePaso(t0, tf, N) #llamo a la funcion
	println("El tamaño de paso es " , h) #mensaje con el tamaño de paso
		else 
		println("El número de pasos no es mayor a cero") #mensaje si N<0
	end
end
#si no es entero positivo entonces marca error
...
function arregloUniforme(t01, tf1, N1, h1)#dar de alta la funcion
if isinteger(N1)
	q= collect(t01:h1:tf1) #arreglo de t
	return q #devuelve q
else
	println("El número de pasos no es un valor entero")
end
...
# ╔═╡ 2f1a1c53-48b1-4b67-9e30-1bab18f1ac19
begin 	
	t01=0   #asiganamos el valor del tiempo inicial
	tf1=5   #asiganamos el valor del tiempo final
	N1=5 #num pasos
	h1=tamañoDePaso(t01, tf1, N1) #llamamos a la funcion para obtener el valor de h1
		if N1>0 #si n>0
		q=arregloUniforme(t01, tf1, N1, h1)
		println("El arreglo uniforme de números desde t0 hasta tf es ", q ,"con un tamaño de paso de ",h1)#arreglo
		else 
		println("El número de pasos no es mayor a cero") #mensaje si N<0
	end
end
...
# Tu código (comentado) va aquí :)
function pasoEuler(ti, xti, g, h) #dar de alta la funcion
	W=	xti + h * g(xti, ti) #ecuacion vista anterior
	return W #devuelve w
end

# ╔═╡ 29ccaac0-69fa-4738-8c4e-ae43ba2d236d
begin
function g(xti, t) #dar de alta la funcion
	return xti #devuelve xti
end
	ti=0   #asiganamos el valor del tiempo inicial
	xti=3   #asiganamos la aprox de x(ti)
	hi=0.4 #tamaño de paso, en este caso lo asignamos
	W=pasoEuler(ti, xti, g, h) #llamamos a la funcion
	println("Aproximación de x(t_{i+1}) es " , W)
end
...
# Tu código (comentado) va aquí :)
function euler(g1, xt0, T) #dar de alta función
N = length(T)
Y = similar(T)
Y[1] = xt0
	for i in 1:N-1 #i en 1 a N-1 
	#iterando para cada i
	ti = T[i] 
	xti = Y[i]
	h = T[i+1] - ti 
	Y[i+1] = pasoEuler(ti, xti, g1, h)
	end
return Y #devuelve y
end

# ╔═╡ b19502f0-4503-4101-ac08-a0cb3fe62a1f
begin
g1(x, t) = x # Función g
xt0 = 7 # condicion (4)
T = [0, 1, 5, 2] # Arreglo tiempo
Y = euler(g, xt0, T) #llamamos a f
println("Aproximación de x(ti): ", Y)
end
...
Si solo dependiera del timepo entonces tendríamos una ecuación diferencial de primer orden, es decir; dt/dt=g(t)(5)
Ahora, tomando en cuenta al metodo euler considerando el intervalo [t0,tf], el tamaño de paso, g, asi como los valores x(ti) entonces tendríamos que llevar a cabo la operación:
$x(t_i+1)=x(t_i)+h*g(t_i)$ para i $\in$ [0,1,2,..,N-1]
...
# Tu código (comentado) va aquí :)
begin
	function metododeeuler(g2, t03, tf3, x03, N3) #dar de alta función
		h = (tf3 - t03) / N3 #expresión vista and
		t3 = range(t03, stop=tf3, length=N3+1)
		x = zeros(Float64, N3+1)
		x[1] = x03
		for i in 1:N3
			x[i+1]= x[i] + h * g2(t3[i], x[i])
		end 
		return t3,x
	end
	function g2(t3,x)
...
# ╔═╡ 1c614712-d75a-4629-8190-bfe6b54e9671
begin 
	t03=0.0 #asiganamos el valor del tiempo inicial
	tf3=5 #asiganamos el valor del tiempo final
	x0=8 #aproximacion
	N3=30 #pasos
	
	t3eul, xeul =metododeeuler(g2, t03, tf3, x0, N3) #metodo euler
	t3_analytic= range(t03, stop=tf3, length=100) #sol analitica t
	x3_analytic= x0*exp.(t3_analytic) #sol analitica x
end

# ╔═╡ ee462fe6-fc92-4312-98f8-0621f8c9ba9c
begin
	plot(t3eul, xeul, marker=:circle, label= "Metodo euler")
	plot!(t3_analytic, x3_analytic, label= "sol. análitica")
	title!("Método Euler")
	xlabel!("t")
	ylabel!("x")
end
