...
# ╔═╡ 9f7f9f39-55c4-4b0a-826d-6225da0c47dd
...
# ╔═╡ dddcc78a-754a-487d-9df4-1aefe6e30cd0
rand()

# ╔═╡ b098bbb6-2c23-4b8a-b1ce-26f677d8f44b
rand(Int)

# ╔═╡ a45572d4-9c8f-4590-a3cd-b001d2cc3df3
rand(Bool)
...
# ╔═╡ 2f298f18-7206-4006-8080-f535b5825d6c
rand(n)

# ╔═╡ 2f867d81-b7b5-4703-8d53-612bf178d97f
rand(Int, n)

# ╔═╡ 0e702d29-7fe6-4eae-99a7-5de2c364466f
rand(Bool, n)
...
# Tu código va aquí :)
function aleatorioUniforme(a, b, n) #dar de alta la función con las variables solicitadas
rand(Uniform(a,b), 1, n) #elegir un num entre a y b en una fila con n columnas
end
...
begin
	histogram([aleatorioUniforme(7,10,30)], bins=4, alpha=0.6, label="a")
end
...
# Tu código va aquí :)
function unPaso() #dar de alta la funcion
	-(-1)^rand(Bool) #valores de salida
end

# ╔═╡ cfa58b0b-2428-4096-a58a-e6c6a122714f
-unPaso()
...
function variosPasos(m)
	rand((-1, 1), 1, m)#arreglo de 1 fila y m columnas
end

# ╔═╡ f78123b5-0bba-4de2-9e78-ed2d6cdc0a30
variosPasos(10)
...
# Tu código (comentado) va aquí :)
function caminataAleatoria(C) #dar de alta la función
	j=zeros(C+1,1) #ceros con c+1 colmnas y una fila 
for i in 2:C+1 #de la entrada m[1,1] a C+1
	j[i, 1]=j[i-1, 1]+ unPaso() #f recursiva
end 
return j #devuelve j
end
...
# Tu código (comentado) va aquí :)
function graficaCaminata(N) #dar de alta función
y=caminataAleatoria(N) #en el eje y
return plot(0:N,y, title="Caminata Aleatoria", xlabel="# pasos", ylabel="posición", label=false) #grafica con inicio en cero hasta N
end

# ╔═╡ 4dccc543-41a8-485d-9a6a-aca07b3d97d0
graficaCaminata(200)
...
# Tu código (comentado) va aquí :)
function graficaCaminata!(M, P) #dar de alta la función
	#M=#caminatas y P=pasos
	plot()
	for i in 1:M
		plot!(0:P, caminataAleatoria(P), title= "caminata aleatoria", xlabel="# pasos", ylabel="posición", lef=false, label=false)
	end 
plot!()
end

# ╔═╡ c4ec89b5-4113-4df5-9524-3a3d43d2fed4
graficaCaminata!(10, 60)
...
# Tu código (comentado) va aquí :)
function animaCaminata(Z) #dar de alta la función
	anim=@animate for i= 1:Z #desde 1 hasta z
		xZ=collect(1:1:Z) #para x
		yZ=caminataAleatoria(200) #para y
		plot(xZ[1:i], yZ[1:i], title= "animaCaminata", xlabel= "# pasos", ylabel="posición", label=false)
	end 
gif(anim, "animaCaminata.gif", fps=1)	
end

# ╔═╡ eecaa6a0-bba1-410d-9d2a-a0fdb133a185
animaCaminata(50)
...
# Tu código (comentado) va aquí :)
function caminataAleatoria2D(Ñ) #dar de alta la función, donde Ñ=cant de pasos
	CD2D=[caminataAleatoria(Ñ) caminataAleatoria(Ñ)]
end

# ╔═╡ 3856a375-3957-496f-bad1-cc8b0e719dd6
...
# Tu código (comentado) va aquí :)
function graficaCaminata2D(R) #dar de alta la función
	n3=caminataAleatoria(R) #para x
	y3=caminataAleatoria(R) #para y
return plot(n3,y3, title= "Caminata 2D", xlabel="Posiciónx", ylabel="posicióny", label=false,aspect_ratio=:equal)
end

# ╔═╡ 7152b189-2541-4724-8221-0582c7668609
graficaCaminata2D(900) 
...
# Tu código (comentado) va aquí :)
function graficaCaminata2D!(M, P) #dar de alta la función
	#M=#caminatas y P=pasos
plot()
	for i in 1:M
		plot!(0:P, caminataAleatoria(P), title="Cinco caminatas", xlabel="# de pasos", ylabel= "posición", label=false, left=false)
	end
		plot!()
end

# ╔═╡ ed80e57b-e38e-456e-aa9f-d59a859376c6
graficaCaminata2D!(5, 60)
...
# Tu código (comentado) va aquí :)
function animaCaminata2D(PP) #dar de alta la función
anim=@animate for i = 1:PP #desde 1 hasta PP
	x2= caminataAleatoria(PP) #para x
	y2= caminataAleatoria(PP) #para y
	plot(x2[1:i], y2[1:i], title="caminata 2D", xlabel= "posición x", ylabel= "posición y", label=false)
end 
gif(anim, "animaCaminata2D.gif", fps=1)
end

# ╔═╡ 6185814e-12b6-4858-94bc-7818f4af420f
animaCaminata2D(10)
...
# Tu código (comentado) va aquí :)
begin
function primgrafica3D(m) #dar de alta la funcion
	D=[0, 0, 0] #caminata inicio
	for i in 2:m 
		cam=[randn(), randn(), randn()] #los pasos
		pto=D[i-1]+cam #definimos una varibale para pasos
		push!(D,pto) #caminata fin
	end
	return D #devuelve D
end
function grafica3D(D)
W= [Z[1] for Z in D] #Para x
V= [Z[2] for Z in D] #Para y
Z= [Z[3] for Z in D] #Para z
return plot(W, V, Z, title="caminata 3D", xlabel= "posicion x", ylabel= "posicion y", zlabel= "posicion z",
...
# ╔═╡ 8c3a3ac2-2a89-4322-af94-9dc3d1de168f
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "4922de94100410ea88595cabe444961edafb0861"
